  ...
  return (
    <main className="m-auto max-w-xl flex flex-col px-2 py-8 h-full">
      <div className="flex-1 flex flex-col gap-4 overflow-auto">
        <h1 className="text-3xl md:text-5xl font-bold text-center">ChatGarbaPT</h1>
        <ul ref={ref} className="flex flex-col flex-1">
          {messages.map((message, i) => (
            <Message message={message} key={message.content + i} />
          ))}
          {messages.length === 0 &&  <li>Pas encore des messages, commencez une conversation!</li>}
          {
            mutation.isLoading && 
            <li className="flex items-center w-full p-4">
              <Loader/>
              <p className="text-gray-300 animate-pulse">ChatGarbaPT est entrain de charger...</p>
            </li>
          }
        </ul>
      </div>
